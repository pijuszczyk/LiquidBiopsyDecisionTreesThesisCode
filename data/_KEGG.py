...
CURRENT_DATASET_PATHS = _KEGG_CNC_16112021_PATHS

USE_MULTITHREADED_LOADING = True
...
def load_matrices(root_dir: Path):
    if CURRENT_DATASET_PATHS.DATA_DIR_NAME is not None:
        labels = load_labels(str(root_dir / CURRENT_DATASET_PATHS.ANNOTATIONS_NAME))
        mats = load_matrices_from_txts(labels.iloc[:, 0], str(root_dir / CURRENT_DATASET_PATHS.DATA_DIR_NAME))
    elif CURRENT_DATASET_PATHS.SINGLE_DATA_FILE_NAME is not None:
        mats = load_matrices_from_single_tsv(str(root_dir / CURRENT_DATASET_PATHS.SINGLE_DATA_FILE_NAME))
    else:
...
if __name__ == '__main__':
    ...
    mats = load_matrices(Path(__file__))
